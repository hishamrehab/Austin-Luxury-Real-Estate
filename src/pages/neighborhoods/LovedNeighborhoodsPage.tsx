...
import { neighborhoodRegionLabel } from '@/pages/neighborhoods/neighborhoodsData'
...
export function LovedNeighborhoodsPage() {
  const { lovedIds, removeLoved, clearLoved, count } = useLovedNeighborhoods()

  const items = lovedIds
    .map((id) => getNeighborhoodDetail(id))
    ...
    <>
      <section className="relative border-b border-charcoal-200/80 bg-gradient-to-b from-[#FAFAF8] via-[#FAFAF8] to-[#F0EFEA] pt-24 pb-14 dark:border-charcoal-800 dark:from-charcoal-950 dark:via-charcoal-950 dark:to-charcoal-900">
        ...
        <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="flex flex-wrap items-end justify-between gap-6">
            <div className="max-w-2xl">
              ...
              <p className="mt-4 max-w-xl text-pretty text-[15px] leading-relaxed text-charcoal-600 dark:text-zinc-400">
                Communities you are weighing. Compare them side by side, then reach out for a curated tour.
              </p>
            </div>
            {items.length > 0 ? (
          ...
          <div className="mt-2 h-px max-w-xs bg-gradient-to-r from-sage-400/50 to-transparent dark:from-sage-500/35" aria-hidden />
          <SavedCollectionTabs />
        </div>
      </section>
          ...
          {items.length === 0 ? (
            <div className="mx-auto max-w-lg rounded-2xl border border-dashed border-charcoal-300/80 bg-white/90 p-12 text-center shadow-sm backdrop-blur-sm dark:border-charcoal-600 dark:bg-charcoal-900/60">
              ...
              <p className="mt-3 text-[15px] leading-relaxed text-charcoal-600 dark:text-zinc-400">
                Tap the heart on any community to add it here. Build a list as you explore.
              </p>
              <Link
                to="/neighborhoods"
            ...
            <div className="grid gap-10 lg:grid-cols-12 lg:gap-12">
              ...
              >
                {items.map((n) => {
                  const region = neighborhoodRegionLabel(n.filters)
                  const median =
                    n.stats.find((s) => s.label === 'Median Price')?.value ??
                    n.fromPrice.replace(/^From\s+/i, '')

                  return (
                    <article key={n.id} className={savedCardClass}>
                      <div className="relative aspect-[4/3] w-full shrink-0 overflow-hidden sm:aspect-[5/3] md:min-h-[220px]">
                        ...
                        >
                          <img
                            alt={n.imageAlt}
                            src={n.image}
                            className="h-full w-full object-cover object-center transition-[transform,filter] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover/card:scale-[1.045] group-hover/card:brightness-[1.02]"
                          />
                        </Link>
                        <div
                        ...
                        <button
                          ...
                          aria-label={`Remove ${n.name} from loved neighborhoods`}
                        >
                      ...
                      <div className="flex flex-1 flex-col border-t border-charcoal-100/90 px-6 pt-6 pb-7 dark:border-charcoal-800/80">
                        <Link
                          to={`/neighborhoods/${n.id}`}
                          className="font-serif text-[1.65rem] font-semibold leading-snug tracking-tight text-charcoal-950 transition-colors hover:text-sage-700 dark:text-zinc-50 dark:hover:text-sage-400"
                        >
                          {n.name}
                        </Link>
                        <div className="mt-5 flex flex-wrap gap-x-4 gap-y-2 text-sm text-charcoal-600 dark:text-zinc-400">
                          <span className="flex items-center gap-1.5">
                            <CircleDollarSign
                              className="size-4 shrink-0 text-sage-600 opacity-80 dark:text-sage-500"
                              aria-hidden
                            />
                            <span className="text-charcoal-500 dark:text-zinc-500">Avg.</span>
                            <span className="font-medium text-charcoal-800 dark:text-zinc-200">{median}</span>
                          </span>
                          <span className="text-charcoal-300 dark:text-charcoal-600" aria-hidden>
                            ·
                          </span>
                          <span className="flex items-center gap-1.5">
                            <Footprints className="size-4 shrink-0 text-sage-600 opacity-80 dark:text-sage-500" aria-hidden />
                            <span className="text-charcoal-500 dark:text-zinc-500">Walk</span>
                            <span className="font-medium text-charcoal-800 dark:text-zinc-200">{n.walkScore}</span>
                          </span>
                          <span className="text-charcoal-300 dark:text-charcoal-600" aria-hidden>
                            ·
                          </span>
                          <span className="flex min-w-0 items-center gap-1.5">
                            <GraduationCap
                              ...
                              aria-hidden
                            />
                            <span className="truncate font-medium text-charcoal-800 dark:text-zinc-200">{n.schools}</span>
                          </span>
                        </div>
                        <Link
                          to={`/neighborhoods/${n.id}`}
                ...
                <div className={savedSummaryClass}>
                  ...
                  <div className="mt-8 border-y border-charcoal-100 py-6 dark:border-charcoal-800">
                    ...
                    <p className="mt-1 text-sm text-charcoal-600 dark:text-zinc-400">
                      {count === 1 ? 'Neighborhood on your list' : 'Neighborhoods on your list'}
                    </p>
                  </div>
